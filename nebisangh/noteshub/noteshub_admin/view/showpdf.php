			...
			<div class="col-md-12">
       		...
       		<tr>
       			...
       			<th data-field="sn">S.N</th>
                            <th data-field="Semester Name">Title</th>
                            <th data-field="sem"> Subject</th>
                            <th data-field="pro"> Provider</th>
                            <th data-field="sem"> Type</th>
                            <th data-field="sem"> Semester Name</th>
       		...
       		<?php
       		$n=0;
       		foreach ($this->pdfname as $ad) {
       			$n++;
                            $unic=$this->uni->selectleveluni($ad->university_id);
                            $levelc= $this->level->selectfacultyuni($ad->level_id);
                            $facc = $this->faculty->selectsemesterfac($ad->faculty_id);
                            $semc = $this->semester->selectsemesterbyid($ad->semester_id);
                            $unid=$unic[0]->name;
                            $leveld = $levelc[0]->level_name;
                            $facd= $facc[0]->faculty_name;
                            $semd= $semc[0]->sem_name;
       			echo "<tr><td>$n</td>";
       			echo "<td>$ad->title</td>";
                            echo "<td>$ad->sub_name</td>";
                            echo "<td>$ad->provider</td>";
                            echo "<td>$ad->type</td>";
                            echo "<td>$semd</td>";
                            echo "<td>$facd</td>";
                            echo "<td>$leveld</td>";
       			...
       			echo "<td> $ad->created_at </td>";
                            ...
                            echo "<td><a href='".base_url()."view/pdfs/".$ad->pdf_notes."'> Download Pdf</a>";
       			
       			if($_SESSION['role']=='SuperAdmin' || $_SESSION['admin']==$ad->created_by)
       			{
       				echo "<td><a href='".base_url()."updatepdf/$ad->id' class='btn btn-primary'>Update</a><a href='".base_url()."deletepdf/$ad->id'' class='btn btn-danger'>Delete </a></td>";
       			}
       			else
       			{
